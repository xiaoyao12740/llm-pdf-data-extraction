...
def extract_fields(pages: list[dict]) -> list[dict]:
    ...
    value_patterns = {
        "report_date": r"\d{4}[-/]\d{2}[-/]\d{2}",
        "period_start": r"\d{4}[-/]\d{2}[-/]\d{2}",
        "period_end": r"\d{4}[-/]\d{2}[-/]\d{2}",
        "region": r"[A-Za-z]+(?:\s+Region)?",
        "sample_count": r"\d[\d,]*",
        "positive_count": r"\d[\d,]*",
        "positive_rate": r"\d+(?:\.\d+)?%",
    }
    for page in pages:
        ...
        for table in page.get("tables", []):
            ...
            for label, raw in zip(headers, values, strict=False):
        ...
        for field, aliases in FIELD_ALIASES.items():
            ...
            match = re.search(
                rf"(?im)^\s*(?:{alias_pattern})\s*(?::|=)?\s*(?:\n\s*)?({value_patterns[field]})\s*$", joined
            )
            if match:
                raw = match.group(1)
                found[field] = ExtractedField(
                    field, raw, _normalize(field, raw), page["page_number"], match.group(0), confidence=0.98
                )
        ...
        if "sample_count" not in found:
            ...
            if match:
                found["sample_count"] = ExtractedField(
                    "sample_count",
                    match.group(1),
                    _normalize("sample_count", match.group(1)),
                    page["page_number"],
                    match.group(0),
                    confidence=0.65,
                )
        if "positive_count" not in found:
            ...
            if match:
                found["positive_count"] = ExtractedField(
                    "positive_count",
                    match.group(1),
                    _normalize("positive_count", match.group(1)),
                    page["page_number"],
                    match.group(0),
                    confidence=0.65,
                )
