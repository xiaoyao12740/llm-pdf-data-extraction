...
def validate_record(record: dict, tolerance: float=.005) -> list[dict]:
    issues=[]
    ...
    if rate is not None and not 0 <= rate <= 1: add("positive_rate","range","positive_rate must be between 0 and 1")
    if samples and positives is not None and rate is not None and abs(rate-positives/samples)>tolerance: add("positive_rate","rate_mismatch","positive_rate is inconsistent with counts")
    if record.get("period_start") and record.get("period_end") and record["period_start"]>record["period_end"]: add("period_start","date_range","period_start is after period_end")
    return issues
