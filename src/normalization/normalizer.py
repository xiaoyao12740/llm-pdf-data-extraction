...
def normalize_value(field, value):
    ...
    if field == "region":
        return " ".join(re.sub(r"\s+region\s*$", "", str(value), flags=re.I).split()).title()
    return str(value).strip()


def fields_to_record(fields):
    return {
        item["field_name"]: normalize_value(item["field_name"], item.get("normalized_candidate", item.get("raw_value")))
        for item in fields
    }
