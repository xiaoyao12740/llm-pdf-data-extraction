...
def _story(record, template, shown_rate, anomaly=None):
    ...
    if anomaly=="date_range": shown_start,shown_end=shown_end,shown_start
    period = f'{shown_start} to {shown_end}'
    if template == "key_value":
        lines = [f'Report Date: {record["report_date"]}', f'Period Start: {shown_start}', f'Period End: {shown_end}', f'Region: {record["region"]}', f'Samples Tested: {record["sample_count"]:,}', f'Positive Cases: {record["positive_count"]:,}', f'Positive Rate: {shown_rate:.2%}']
    elif template == "aliases":
        lines = [f'Generated On: {record["report_date"]}', f'From: {shown_start}', f'To: {shown_end}', f'Area: {record["region"]}', f'Total Tests: {record["sample_count"]:,}', f'Detected Positive: {record["positive_count"]:,}', f'Detection Rate: {shown_rate:.2%}']
    elif template == "multiline":
        lines = [f'Report Date\n{record["report_date"]}', f'Period Start\n{shown_start}', f'Period End\n{shown_end}', f'Region\n{record["region"]}', f'Number Tested\n{record["sample_count"]:,}', f'Positives\n{record["positive_count"]:,}', f'Rate\n{shown_rate:.2%}']
    elif template == "narrative":
        lines = [f'Report Date: {record["report_date"]}', f'Period Start: {shown_start}', f'Period End: {shown_end}', f'Region: {record["region"]}', f'During {period}, laboratories processed a total of {record["sample_count"]:,} samples in {record["region"]}. Of which {record["positive_count"]:,} were positive.', f'Positive Rate: {shown_rate:.2%}']
    else:
        data = [["Report Date", "From", "To", "Region", "Tests", "Positive", "Rate"], [record["report_date"], shown_start, shown_end, record["region"], f'{record["sample_count"]:,}', f'{record["positive_count"]:,}', f'{shown_rate:.2%}']]
        if anomaly=="missing_field": data=[row[:-1] for row in data]
        ...
        return title + [table]
    if anomaly=="missing_field": lines=[line for line in lines if "Rate" not in line]
...
def generate(count=100, seed=42, raw_dir=None, truth_path=None):
    ...
    for i in range(count):
        ...
        anomaly = "date_range" if i%29==0 else "missing_field" if i%23==0 else "rate_mismatch" if i%17==0 else None; shown_rate=min(1, rate+.02) if anomaly=="rate_mismatch" else rate
        ...
        name=f"report_{i+1:03d}_{template}.pdf"; SimpleDocTemplate(str(raw/name), pagesize=A4).build(_story(rec, template, shown_rate, anomaly))
